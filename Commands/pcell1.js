module.exports = {
    name: 'pcell1',
    description: 'TEQ UR Perfect Cell without EZA',
    ...
    execute(message, args) {
        ...
        var url = "https://dbz-dokkanbattle.fandom.com/wiki/Surpassing_All_Perfect_Cell";
        ...
        var leader = "<:teq:729467504593862728> Type Ki +3, ATK +3000";
        var superatk = "Solar Kamehameha: Causes supreme damage to enemy";
        var passive = "Avatar of Contempt: ATK & DEF +12% for every Ki Sphere obtained";
        var stats = "HP: 12,592 (55%)/16,232 (100%)\nATK: 11,232 (55%)/15,992 (100%)\nDEF: 8,834 (55%)/13,034 (100%)";
        const {link} = require('./pcell1.js')
        var links = "";
            for (var i = 0; i < link.length; i++)
            {
                links += link[i] + "\n";
            }
        const {categories} = require('./pcell1.js')
        var cats = "";
            for (var i = 0; i < categories.length; i++)
            {
                cats += categories[i] + "\n";
            }
        var apt = "APT: 1,891,074 (unsupported)/2,307,364 (supported)\nDefense: 102,081\nLinking Partner: [INT LR Cell (Perfect Form)](https://dbz-dokkanbattle.fandom.com/wiki/The_True_Value_of_Perfect_Form_Cell_(Perfect_Form)) \nTeam: [Androids](https://dbz-dokkanbattle.fandom.com/wiki/Androids) \nBuild: 6 Additional/20 Critical";
        ...
        var footnotes = "";
        const {status} = require('./pcell1.js')
        if (status === 'incomplete')
        {
            message.channel.send({embed: {
                ...
                title: "Perfect Cell is coming soon."
            }});
        }
        else
        {
