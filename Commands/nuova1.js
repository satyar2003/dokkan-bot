...
module.exports = {
    name: 'nuova1',
    description: 'PHY UR Nuova Shenron',
    categories: ['', ''],
    link: ['', ''],
    status: 'incomplete',
    plural: 'false',
    aliases: ['Nuova Shenron'],
    execute(message, args) {
		const pfp = message.author.displayAvatarURL({format: "png", dynamic: "true"})
        const {link} = require('./nuova1.js')
        const {categories} = require('./nuova1.js')
        const {status} = require('./nuova1.js')
        const {plural} = require('./nuova1.js')
        const {aliases} = require('./nuova1.js')
        var color = 7164715;
        var name = "";
        var url = "";
        var desc = "";
