module.exports = {
    ...
    execute(message, args) {
        ...
        else
        {
            ...
            for (var k = 0; k < command.categories.length; k++)
            {
                for (var l = 0; l < command2.categories.length; l++)
                {
                    if (command.categories[k] == command2.categories[l])
                    {
                        shared += command.categories[k] + "\n";
                        num++;
                    }
                }
            }
            ...
            else
            {
                message.channel.send({embed: {
                    ...
                    {
                        name: message.author.username,
                        icon_url: message.author.displayAvatarURL({format: "png", value: "true"})
                    },
                    title: `**${command.aliases[command.aliases.length - 1]}** and **${command2.aliases[command2.aliases.length - 1]}** do not share any categories`,
                    timestamp: new Date()
                }});
            }
        }
