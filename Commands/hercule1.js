...
module.exports = {
    name: 'hercule1',
    description: 'AGL UR Hercule/Majin Buu (Good) without EZA',
    categories: ['', ''],
    link: ['', ''],
    status: 'incomplete',
    plural: 'true',
    aliases: ['Hercule/Majin Buu (Good)'],
    execute(message, args) {
		const pfp = message.author.displayAvatarURL({format: "png", dynamic: "true"})
        const {link} = require('./hercule1.js')
        const {categories} = require('./hercule1.js')
        const {status} = require('./hercule1.js')
        const {plural} = require('./hercule1.js')
        const {aliases} = require('./hercule1.js')
        var color = 2317729;
        var name = "";
        var url = "";
        var desc = "";
